suite_builderUI <- function(id, platform) {
  ...
  shiny::tagList(
    ...
    hr(),
    br(),
    h2("Prepare Files for Import"),
    "Upload suite files (multiple files accepted) to remove the 'Model Agg' column and combine into a single file ready for Orion import.",
    shiny::tags$div(
      style = "color: #888888; font-style: italic;",
      "Note: The Orion import is done under the Sleeve Strategy app, in the Tools menu under 'Sleeve Strategy Import'."
    ),
    br(),
    br(),
    fluidRow(
      ...
      column(
        width = 2,
        br(),
        shiny::uiOutput(ns("downloadProcessedUI"))
      )
    )
  )
}
...
suite_builderServer <- function(id, platform) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    crack_name <- function(suite_name, w) {
      ...
      if (is_tm) {
        suite_name <- stringr::str_remove(suite_name, " TM")
      }
    ...
    # File processing for import preparation
    processed_data <- reactive({
      shiny::req(input$uploadFiles)

      # Process each uploaded file
      all_data <- purrr::map_dfr(input$uploadFiles$datapath, function(file_path) {
        tryCatch(
          {
            # Read the Excel file
            wb <- openxlsx::loadWorkbook(file_path)
            data <- openxlsx::readWorkbook(wb, sheet = 1, colNames = TRUE)

            # Get the original column names from the Excel file (first row)
            original_names <- openxlsx::read.xlsx(file_path, rows = 1, colNames = FALSE) |>
              unlist() |>
              as.character()

            # Assign the original column names to preserve spaces
            if (length(original_names) == ncol(data)) {
              names(data) <- original_names
            }

            # Remove the "Model Agg" column (column B)
            if ("Model Agg" %in% names(data)) {
              data <- data |> dplyr::select(-`Model Agg`)
            }
          ...
          error = function(e) {
            showNotification(
              paste("Error processing file:", basename(file_path), "-", e$message),
              type = "error"
            )
            return(NULL)
          }
    ...
    # Conditional UI for download button
    output$downloadProcessedUI <- renderUI({
      if (!is.null(input$uploadFiles) && nrow(processed_data()) > 0) {
        shiny::downloadButton(session$ns("downloadProcessed"), "Download Processed File")
      }
    })

    output$downloadProcessed <- downloadHandler(
      filename = function() {
        # Get base name from first uploaded file
        if (!is.null(input$uploadFiles)) {
          base_name <- tools::file_path_sans_ext(input$uploadFiles$name[1])
          paste0(base_name, "_ready_for_import.xlsx")
        } else {
          "processed_suite_ready_for_import.xlsx"
        }
      },
      content = function(file) {
        data <- processed_data()
        ...
        openxlsx::write.xlsx(x = data, file = file, colNames = TRUE, rowNames = FALSE)
      }
    )
  })
}
